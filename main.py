...
async def _auto_approve_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Auto-approve submission if admin took no action within 1 minute."""
    job = context.job
    user_id = job.data["user_id"]
    sub_id = job.data["sub_id"]
    admin_msg_ids = job.data.get("admin_msg_ids", {})  # {admin_id: message_id}
    task_price = get_task_price()

    # Check if submission still exists (not already reviewed)
    sub = db.reference(f"submissions/{user_id}/{sub_id}").get()
    if not sub:
        return  # Already approved/cancelled by admin
...
async def handle_account_registered(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # ── Schedule auto-approve after 60 seconds ──
    context.job_queue.run_once(
        _auto_approve_job,
        when=60,
        data={"user_id": user.id, "sub_id": sub_id, "admin_msg_ids": admin_msg_ids},
        name=f"auto_approve_{user.id}_{sub_id}",
    )
...
async def callback_quick_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Cancel the auto-approve job if still pending
    job_name = f"auto_approve_{user_id}_{sub_id}"
