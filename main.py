...
async def handle_new_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "📱  Instagram 2FA Method\n"
        "  👉 [t.me](https://t.me/asklybux\n\n)"
        "🍪  Facebook Cookie Method\n"
        "  👉 [t.me](https://t.me/SaaFe_Bio\n\n)"
        f"{DIV_SHORT}\n\n"
...
async def handle_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "  📥  Withdraw\n"
        f"{DIV}\n\n"
        f"Available Balance:  {fmt_usd(balance)}\n\n"
...
async def handle_task_fb_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        f"Password:    `{password}`\n\n"
        f"{DIV_SHORT}\n\n"
        "Log in to Facebook with these\n"
        "credentials on a real device.\n\n"
        "Then send your Facebook Account UID\n"
        "(numeric ID) below.\n\n"
        f"{DIV}",
        parse_mode=None,
        reply_markup=ReplyKeyboardMarkup([["🔙 Cancel"]], resize_keyboard=True),
...
async def handle_fb_await_uid(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not text:
        await update.message.reply_text("Please send your Facebook Account UID.")
        return TASK_FB_AWAIT_UID
