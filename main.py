...
async def handle_2fa_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        f"3. Enter password: <code>axiex@4</code>\n"
        f"4. Enter this code\n\n"
...
async def _send_live_page(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int):
    """Send a specific page of live users list."""
    ...
    # Build message text
    lines = [
        f"📊 **ACTIVE USERS** (Who submitted at least 1 account)",
        f"📄 Page {page}/{total_pages}",
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        ""
    ]
    ...
    # Build inline keyboard for User IDs (3 per row)
    keyboard = []
    row = []
    for user in page_users:
        row.append(InlineKeyboardButton(str(user['user_id']), callback_data=f"live_copy_uid:{user['user_id']}"))
        if len(row) == 3:
            keyboard.append(row)
            row = []
    if row:
...
async def callback_live_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle live list pagination and copy callbacks."""
    query = update.callback_query
    user_id = query.from_user.id
...
async def cmd_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "📊 **STATS**\n"
        "/live — Active users (who submitted)\n"
        "/stats — Bot statistics\n"
        "/ldset — Real leaderboard\n"
        "/ldauto — Auto leaderboard\n\n"
