...
def validate_2fa_key(raw: str) -> tuple:
    """
    Validate and normalise a TOTP 2FA key.

    Returns (normalised_key, error_message).
    error_message is None when valid.
    """
    cleaned = raw.replace(" ", "").upper()
    if len(cleaned) != 16:
        return None, (
            f"❌ Invalid 2FA key length. Must be exactly 16 characters "
            f"(you entered {len(cleaned)} after removing spaces). Try again:"
        )
    if not _BASE32_RE.match(cleaned):
        return None, (
            "❌ Invalid 2FA key format. Only letters A–Z and digits 2–7 are allowed "
            "(no special characters, 0, 1, 8, or 9). Try again:"
        )
    return cleaned, None
