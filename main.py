...
async def handle_2fa_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Create INLINE keyboard with copy button (1-click copy)
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📋 Copy 6-Digit Code", callback_data=f"copy_otp:{otp_code}")],
        [InlineKeyboardButton("✅ Account Registered", callback_data="confirm_registered")],
        [InlineKeyboardButton("❌ Cancel Task", callback_data="cancel_2fa_task")]
    ])
    
    await update.message.reply_text(
        f"🔐 **Your 6-Digit Code:**\n"
        f"`{otp_code}`\n\n"
        f"💡 **Click 'Copy Code' button below to copy!**\n\n"
        f"Then click 'Account Registered' after login.",
        parse_mode=None,
...
async def callback_2fa_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle 2FA inline keyboard callbacks (copy, confirm, cancel)."""
    query = update.callback_query
    data = query.data
    
    # Handle copy OTP (1-click copy)
    if data.startswith("copy_otp:"):
        otp_code = data.split(":")[1]
        await query.answer(f"✅ Copied: {otp_code}", show_alert=False)
        await query.message.reply_text(
